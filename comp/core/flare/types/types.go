...
// DefaultFiller wraps the callback with the default FlareProvider configuration
func DefaultFiller(callback FlareCallback) *FlareFiller {
...
func NewProvider(callback FlareCallback) Provider {
	return Provider{FlareFiller: DefaultFiller(callback)}
}
